...
const isgroupStart = syntax.isgroupStart,
...
function tag(tagname) {
  return function fn(content, attr) {
    ...
    if (typeof attr !== "object") {
      ...
      const attrstr = Object.keys(attr).map(function(key) {
        return `${key}="${attr[key]}"`;
      }).join(" ");
...
function ascii2mathml(asciimath, options) {
  ...
  const math = options.display !== "inline" ?
          function(expr) {
...
function makeParse(options) {
  ...
  function parseone(ascii, grouped, lastel) {
    ...
    if (ascii.startsWith("sqrt")) {
      ...
      let split = syntax.splitNextWhitespace(ascii, options);
      ...
      let [, open, group, close, after] = isgroupStart(ascii) ?
