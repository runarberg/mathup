...
function ascii2mathml(asciimath, options) {
  ...
  if (options.annotate) {
    ...
    let mathml = parsed === getlastel(parsed) ? parsed : mrow(parsed);
...
function makeParse(options) {
  ...
  let decimalMarkRE = options.decimalMark === "." ? "\\." : options.decimalMark;
  let numberRegexp = new RegExp("^\\d+("+ decimalMarkRE +"\\d+)?");
